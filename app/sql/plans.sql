...
CREATE TABLE IF NOT EXISTS plans.plans(
    ...
    bible_canon smallint NOT NULL DEFAULT 0, -- for catholic and orthodox bible | defaults to 0 which is the 'normal' bible
    length int NOT NULL DEFAULT 820847, -- sum of all (HELPER: instead of sum(length) + double join)
...
CREATE TABLE IF NOT EXISTS plans.bible_plans(
    ...
    plan_fk int REFERENCES plans.plans ON DELETE CASCADE NOT NULL,
    chapter_fk int2 REFERENCES static.chapters NOT NULL,
    verse_fks smallint[], -- optional
